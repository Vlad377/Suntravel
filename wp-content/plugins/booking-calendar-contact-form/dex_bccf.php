...
Plugin URI: http://bccf.dwbooster.com
...
Version: 1.0.84
Author: CodePeople
Author URI: http://bccf.dwbooster.com/download
License: GPL
*/
...
define('DEX_BCCF_DEFAULT_DEFER_SCRIPTS_LOADING', (get_option('CP_BCCF_LOAD_SCRIPTS',"0") == "1"?true:false));
...
function dex_bccf_settingsLink($links) {
    $settings_link = '<a href="admin.php?page=dex_bccf.php">'.__('Settings','bccf').'</a>';
...
function dex_bccf_helpLink($links) {
    $help_link = '<a href="https://wordpress.org/support/plugin/booking-calendar-contact-form#new-post">'.__('Support','bccf').'</a>';
	array_unshift($links, $help_link);
	$s_link = '<a href="http://bccf.dwbooster.com/support?priority=1">'.__('Documentation','bccf').'</a>';
	array_unshift($links, $s_link);
	return $links;
}

function dex_bccf_customAdjustmentsLink($links) {
    $customAdjustments_link = '<a href="http://bccf.dwbooster.com/download">'.__('Upgrade To Premium','bccf').'</a>';
...
function dex_bccf_html_post_page() {
    ...
    else
    {
        ...
        {
            echo("Redirecting to upgrade page...<script type='text/javascript'>document.location='http://bccf.dwbooster.com/download';</script>");
            exit;
        } 
        else if (isset($_GET["page"]) &&$_GET["page"] == 'dex_bccf_demo')
        {
            echo("Redirecting to demo page...<script type='text/javascript'>document.location='http://bccf.dwbooster.com/home#demos';</script>");
            exit;
        } 
        else        
...
add_action( 'plugins_loaded', 'dex_bccf_check_posted_data', 11 );
...
function dex_bccf_check_posted_data()
{
    ...
    if (!$rows_affected)
    {
        ...
        echo '<br /><br />If the error persists  please be sure you are using the latest version and in that case contact support service at http://bccf.dwbooster.com/contact-us?debug=db';
        exit;
    }
...
function dex_bccf_check_upload($uploadfiles) {
    ...
    if ( in_array ($filetype["ext"],array("php","asp","aspx","cgi","pl","perl","exe","js","com","bat","msi","ini")) )
        return false;
    else
        return true;
}
...
function dex_bccf_get_option ($field, $default_value)
{
    ...
    {
       $myrows = $wpdb->get_results( "SELECT * FROM ".DEX_BCCF_CONFIG_TABLE_NAME." WHERE id=".$calid );
