...
if (isset($_GET['u']) && $_GET['u'] != '')
{
    ...
    {    
        $wpdb->query('UPDATE `'.DEX_BCCF_CONFIG_TABLE_NAME.'` SET conwer='.intval($_GET["owner"]).',`'.TDE_BCCFCALDELETED_FIELD.'`='.intval($_GET["public"]).',`'.TDE_BCCFCONFIG_USER.'`="'.esc_sql($_GET["name"]).'" WHERE `'.TDE_BCCFCONFIG_ID.'`='.intval($_GET['u']));           
...
<?php if (cp_bccf_is_administrator()) { ?> 
 ...
 <div id="metabox_basic_settings" class="postbox" >
  ...
  <div class="inside"> 
    ...
    This version supports one calendar. For a version that supports unlimited calendars upgrade to the <a href="http://bccf.dwbooster.com/download">pro version</a>.
